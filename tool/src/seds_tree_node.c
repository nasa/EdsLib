...
static int seds_tree_mark_error(lua_State *lua)
{
    ...
    seds_user_message_preformat(msgtype, lua_tostring(lua, -2), lua_tointeger(lua, -1), lua_tostring(lua, 2), extramsg);
...
static int seds_tree_node_get_property(lua_State *lua)
{
    ...
    if (lua_type(lua, 2) == LUA_TSTRING && strcmp(lua_tostring(lua, 2), "entity_type") == 0)
    {
        if (pnode->node_type < SEDS_NODETYPE_MAX && SEDS_NODETYPE_LOOKUP[pnode->node_type] != NULL)
        {
...
static int seds_tree_node_new_object(lua_State *lua)
{
    ...
    if (nodetype_str != NULL)
    {
        ...
        {
            if (SEDS_NODETYPE_LOOKUP[i] != NULL && strcmp(SEDS_NODETYPE_LOOKUP[i], nodetype_str) == 0)
            {
