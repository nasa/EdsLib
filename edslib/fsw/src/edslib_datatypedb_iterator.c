        ...
        switch (NextAction)
        {
            ...
            case EDSLIB_ITERATOR_RC_CONTINUE:
                         ...
                         || ParentLev->DataDictPtr->BasicType == EDSLIB_BASICTYPE_COMPONENT)
                {
                    ...
                    CurrLev->Details = ParentLev->DataDictPtr->Detail.Container->EntryList[CurrLev->CurrIndex];
                    ...
                    if (CurrLev->CurrIndex == 0)
                    {
                         ...
                         * added now.
                         */
                        CurrLev->StartOffset.Bytes = ParentLev->StartOffset.Bytes + CurrLev->Details.Offset.Bytes;
                        CurrLev->StartOffset.Bits  = ParentLev->StartOffset.Bits + CurrLev->Details.Offset.Bits;
                    }
                    else
                    {
                    ...
                    if (CurrLev->CurrIndex < (ParentLev->DataDictPtr->NumSubElements - 1))
                    {
                        ...
                        CurrLev->EndOffset =
                            ParentLev->DataDictPtr->Detail.Container->EntryList[1 + CurrLev->CurrIndex].Offset;
