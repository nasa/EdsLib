...
{
    EdsLib_DataTypeDB_EntityInfo_t
                EntityInfo; /**< Information about the component itself (type ID, offset within parent) */
...
extern "C"
{
     ...
     */
    const char *EdsLib_DisplayDB_GetTypeName(const EdsLib_DatabaseObject_t *GD,
                                             EdsLib_Id_t                    EdsId,
                                             char                          *Buffer,
                                             uint32_t                       BufferSize);
     ...
     */
    const char *
    EdsLib_DisplayDB_GetNameByIndex(const EdsLib_DatabaseObject_t *GD, EdsLib_Id_t EdsId, uint16_t SubIndex);
     ...
     * in the parameter descriptor will always be just a single (short) element name.  An element that is a sub
     * structure will have the "SubStructureId" value set to a nonzero value and array elements will only get a single
     * callback for the first element of the array.
     *
     * @param GD the active EdsLib runtime database object
     ...
     *
     * This version will descend into all sub structures and name matching is performed on the fully qualified name
     * which should be in C-style syntax.  For instance:
     *
     ...
     * @param DisplayOffset the offset of the first byte in the data block.  This is typically zero but may be nonzero
     * if the data should be viewed as a continuation of a previous display.
    ...
    /**
     * Convert a binary/machine value into a printable string using the EdsLib_DisplayHint_t as guidelines for how to
     * interpret the data.
     *  - For character strings, printable chars (as indicated by isprint()) will copied to the output buffer and null
     * termination is ensured.
     *  - For integer numbers, these will be converted to symbolic constants if the "EnumNamespace" is set and a
     * matching value is found.
     *  - For numbers for which a symbolic constant is not defined, conversion will fall back to an appropriate
     ...
     * @param SrcPtr pointer to binary/machine data.  Typically this is a pointer to a byte offset inside an "unpacked"
     * message or structure, retrieved via the "MemberAbsOffset" field as returned from a locate or walk routine.
    ...
    /**
     * Convert string into binary/machine data using the EdsLib_DisplayHint_t as guidelines for how to interpret the
     * data.
     *  - For character strings, all chars will copied to the destination up to the size limit or the terminating NULL
     *  - For integer numbers, these may be symbolic constants if the "EnumNamespace" is set and a matching value is
     * found.
     *  - For numbers for which a symbolic constant is not defined, conversion will fall back to an appropriate
     * strtol/ul/d/f system call
     *
     * EdsLib_Scalar_ToString() will perform the reverse operation
     *
     * @param GD the active EdsLib runtime database object
     * @param DestPtr pointer to binary/machine data.  Typically this is a pointer to a byte offset inside a message or
     * structure, retrieved via the "MemberAbsOffset" field as returned from a locate or walk routine.
     * @param SrcString The null-terminated string to convert into machine data
     * @returns EDSLIB_SUCCESS if successful, error code if conversion failed.
     *
     * \sa EdsLib_Scalar_ToString()
     */
    int32_t EdsLib_Scalar_FromString(const EdsLib_DatabaseObject_t *GD,
                                     EdsLib_Id_t                    EdsId,
                                     void                          *DestPtr,
