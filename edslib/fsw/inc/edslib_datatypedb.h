...
extern "C"
{
     ...
     *
     * SubMsgId will be set to an invalid identifier if the index is out of range, or the parent does not have any
     * members
     *
     * @param GD the runtime database object
    ...
    /**
     * Given a type with sub-members (container, array, interface, etc), look up the identification and offset for
     * the immediate sub-member containing the item at the given bit offset.  This only locates the _immediate_
     * subelement containing the offset, which could be another container.  This function may have to be called multiple
     * times to locate a single item.
     *
     ...
     */
    int32_t
    EdsLib_DataTypeDB_FinalizePackedObject(const EdsLib_DatabaseObject_t *GD, EdsLib_Id_t EdsId, void *PackedData);
    ...
    /**
     * Given an unpacked (native byte order) container, identify the derived contents using EDS-specified constraint
     * values
     *
