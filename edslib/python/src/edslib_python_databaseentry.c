...
static PySequenceMethods EdsLib_Python_DatabaseEntry_SequenceMethods = { .sq_length = EdsLib_Python_DatabaseEntry_len,
                                                                         .sq_item =
                                                                             EdsLib_Python_DatabaseEntry_seq_item };

static PyMappingMethods EdsLib_Python_DatabaseEntry_MappingMethods = { .mp_length = EdsLib_Python_DatabaseEntry_len,
                                                                       .mp_subscript =
                                                                           EdsLib_Python_DatabaseEntry_map_subscript };
...
PyTypeObject EdsLib_Python_DatabaseEntryType = { PyVarObject_HEAD_INIT(NULL, 0).tp_name =
                                                     EDSLIB_PYTHON_ENTITY_NAME("DatabaseEntry"),
                                                 ...
                                                 .tp_weaklistoffset =
                                                     offsetof(EdsLib_Python_DatabaseEntry_t, WeakRefList),
                                                 .tp_doc = PyDoc_STR("EDS database entry") };

PyTypeObject EdsLib_Python_EnumEntryIteratorType = { PyVarObject_HEAD_INIT(NULL, 0).tp_name =
                                                         EDSLIB_PYTHON_ENTITY_NAME("EnumerationEntryIterator"),
...
PyTypeObject EdsLib_Python_ContainerEntryIteratorType = { PyVarObject_HEAD_INIT(NULL, 0).tp_name =
                                                              EDSLIB_PYTHON_ENTITY_NAME("ContainerEntryIterator"),
...
static void
EdsLib_Python_DatabaseEntry_GetFormatCodes(char *buffer, const EdsLib_Python_Database_t *refdb, EdsLib_Id_t EdsId)
{
    ...
    if (RepeatCount > 1)
    {
        snprintf(buffer, EDSLIB_PYTHON_FORMATCODE_LEN, "%u%c", (unsigned int)RepeatCount, code);
    }
    else
    {
        buffer[0] = code;
...
PyTypeObject *EdsLib_Python_DatabaseEntry_GetFromEdsId_Impl(EdsLib_Python_Database_t *refdb, EdsLib_Id_t EdsId)
{
    ...
    do
    {
         ...
         */
        self = (EdsLib_Python_DatabaseEntry_t *)EdsLib_Python_GetFromCache(refdb->TypeCache,
                                                                           edsidval,
                                                                           &EdsLib_Python_DatabaseEntryType);
        if (self != NULL)
        {
            break;
        }
        ...
        else if (TypeInfo.ElemType == EDSLIB_BASICTYPE_FLOAT || TypeInfo.ElemType == EDSLIB_BASICTYPE_SIGNED_INT
        ...
        if (TypeInfo.NumSubElements > 0)
        {
            ...
            EdsLib_DisplayDB_IterateBaseEntities(refdb->GD,
                                                 EdsId,
                                                 EdsLib_Python_DatabaseEntry_build_dict_callback,
                                                 tempargs);
            Py_DECREF(tempargs);
            tempargs = NULL;
         ...
         */
        self = (EdsLib_Python_DatabaseEntry_t *)
                   EdsLib_Python_DatabaseEntryType.tp_base->tp_new(&EdsLib_Python_DatabaseEntryType, tempargs, NULL);
        Py_DECREF(tempargs);
        tempargs = NULL;
        if (self == NULL)
        {
...
static PyObject *EdsLib_Python_DatabaseEntry_seq_item(PyObject *obj, Py_ssize_t idx)
{
    ...
    if (TypeInfo.NumSubElements == 0)
    {
        PyErr_Format(PyExc_TypeError,
                     "Attempt to index \'%s\' which is not an EDS sequence type",
...
static PyObject *EdsLib_Python_DatabaseEntry_repr(PyObject *obj)
{
    ...
    result = PyUnicode_FromFormat("%s(%R,%R)", obj->ob_type->tp_name, self->EdsDb->DbName, self->EdsTypeName);
...
static PyObject *EdsLib_Python_DatabaseEntry_iter(PyObject *obj)
{
    ...
    switch (DisplayHint)
    {
        ...
        case EDSLIB_DISPLAYHINT_MEMBER_NAMETABLE:
            ...
            ContainerIter =
                PyObject_GC_New(EdsLib_Python_ContainerIterator_t, &EdsLib_Python_ContainerEntryIteratorType);
        ...
        default:
            return PyErr_Format(PyExc_TypeError,
                                "%s is not an EDS Iterable type (Container or Enumeration)",
                                Py_TYPE(obj)->tp_name);
    }
...
static PyObject *EdsLib_Python_EnumEntryIterator_iternext(PyObject *obj)
{
    ...
    do
    {
        ...
        Label = EdsLib_DisplayDB_GetEnumLabelByIndex(dbent->EdsDb->GD,
                                                     dbent->EdsId,
                                                     self->Index,
                                                     LabelBuffer,
                                                     sizeof(LabelBuffer));
